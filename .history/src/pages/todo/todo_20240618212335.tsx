...
const Todo = () => {
  ...
  const handleDragEnd = ({ active, over }) => {
    if (active.id !== over.id) {
      setTodos((prevTodos) => {
        ...
        if (updatedTodo) {
          updatedTodo.Status = findStatus(over.id);
        }
  ...
  const findStatus = (id) => {
    if (todos.find(todo => todo.Id === id)?.Status === 'Todo') {
      return 'Doing';
    } else if (todos.find(todo => todo.Id === id)?.Status === 'Doing') {
      return 'Done';
    } else if (todos.find(todo => todo.Id === id)?.Status === 'Done') {
      return 'Todo';
    }
  };
